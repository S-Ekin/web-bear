...
export default class ComTreeBox extends React.PureComponent<props, states> implements ComTreeboxSpace.comTreeboxAPI {
	...
	componentWillReceiveProps(nextProp: props) {
		...
		//父组件
		if (nextProp.data != this.props.data || nextProp.defaultSel!=this.props.defaultSel || this.props.init!=nextProp.init) {
	...
	render() {
		...
		const { maxHeight, pannelWidth ,is_combo} = this.props;
		return (
				<div className="m-drop" style={{ width: (pannelWidth ? pannelWidth : "100%") ,position:!is_combo?"relative":"absolute"}}>
