...
export default class CalendarDayView extends React.PureComponent<Props, States>
  ...
  getDayArr () {
    ...
    const daysArr = new Array(dayArrleg).fill("1")
      .map((...args) => {
        ...
        switch (index) {
          ...
          case 0: {  // 日历的第一行，有可能包含上个月的
            ...
            const preMonDayArr = Array.from({ length: monFirstDayToWeek }, (...monArgs) => monArgs[1] + 1).map((val) => {
            ...
            const firstArr = Array.from({ length: 7 - monFirstDayToWeek }, (...MonArgs) => MonArgs[1] + 1).map((val) => (
              <DayComponent
                dayNum={val}
                key={val}
                disabled={false}
          ...
          case dayArrleg - 1: {// 最后一排，可能包含下个月的日期
            ...
            const lastArr = Array.from({ length: count }, (...MonArgs) => MonArgs[1] + 1).map((val) => {
            ...
            const lastMonDays = Array.from({ length: 7 - count }, (...monArgs) => monArgs[1] + 1).map((val) => (
              <DayComponent
                dayNum={val}
                disabled
                curTime={curTime}
          ...
          default: {
            ...
            const monDayArr = Array.from({ length: 7 }, (...monArgs) => monArgs[1] + 1).map((val) => {
              const day = val + startDayNum;
              return (
                <DayComponent
                  dayNum={day}
