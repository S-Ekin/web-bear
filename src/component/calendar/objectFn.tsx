...
const timeStrValToTimeObjArr = function (
  ...
  if (selTimeValStr) {
    ...
    const  errorFormate = selTimeValStr.split(",").some((val) => !reg.test(val));
    if (errorFormate) {
      selTimeValStr = "";
    }
  }
  ...
  const selTimeArr = curTimeArr.map((val, index) => !selTimeValStr ? val : setTime(defaultTimeArr[index])!);
...
const getInpTimeStrArr = function (
  ...
  const listArr = selTimeArr.map((val) => getStr(val)!).toJS();
...
const	timeStrToNumber = function (strArr:string[], valFormatt:"number"|"string") {
  return valFormatt === "number" ? strArr.map((val) => val.replace(/-/g, "").replace(/:/g, "")
    .replace(/\s/g, "")) : strArr;
};
