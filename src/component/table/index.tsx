...
const initCurPage = <T extends AnyObj>(
  props: Props<T>,
  perNums: number,
  curOptId?: string
) => {
  ...
  if (!curOptId) {
    ...
    const index = data.findIndex(
      (val) => String(val[idField!]) === String(curOptId)
    );
    const pages = index > -1 ? Math.ceil((index + 1) / perNums) : 1;
    return pages;
  }
};
class Table<T extends AnyObj>
  ...
  static colItem = <K extends AnyObj>({
    ...
    children,
    align = "td-left",
  }: React.PropsWithChildren<IColumnItem<K>>) => (
    <th style={{ width: width }} className={`${align}`}>
      {children}
    </th>
  );
  ...
  initFixObj () {
    ...
    const column: IColumnItem<T>[] = arr
      .filter((val) => val)
      .map((val) => {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { children, ...obj } = val!.props as {
  ...
  scrollToSeletItem () {
    ...
    const item = this.tableContainer.current!.querySelector(
      ".selected"
  ...
  countCurPageTotalStatus () {
    ...
    if (tableData.size === 0) {
      ...
      if (allCheck) {
        ...
        return hasCheck
          ? checkAllStataus.hasChecked
          : checkAllStataus.noChecked;
      }
    }
  }
  ...
  getFixHead () {
    ...
    return (
      <table>
        ...
        <thead>
          <tr>
            <th key="order" className="td-left">
              {checkCom}序号
            </th>
            {children}
          </tr>
    ...
    return (
      <div className={`g-table-wrap ${borderName}`} style={style}>
        <div className="g-table" ref={this.tableContainer}>
          <div className={"m-fixTabHead "}>{this.getFixHead()}</div>
          {mianBody}
          {pageSize}
        </div>
      </div>
