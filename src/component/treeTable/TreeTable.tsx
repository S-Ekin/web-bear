...
type Props<T> = {
  ...
  clickOrCheckForbid?: (
    node: IImmutalbeMap<T & Inode>,
    field?: string
  ) => boolean;
  bindGetSelectedFn?: (
    getSelected: () => IImmutalbeList<IImmutalbeMap<T & Inode>>
...
class TreeTable<T extends AnyObj>
  extends React.PureComponent<Props<T>, States<T>>
  implements ITreeTable<T> {
  static defaultProps = {
    idField: "id",
    childField: "children",
    defaultSel: "",
  ...
  constructor (props: Props<T>) {
    ...
    const obj = formatterTreeData(
      { childField: childField!, idField, multiply },
      defaultSel!,
      data
    );
  ...
  expand (path: string) {
    ...
    this.setState((pre) => ({
      immutabData: pre.immutabData.updateIn(
        arr,
        (val: IImmutalbeMap<ICommon<T>["node"]>) => {
  ...
  checkChild (path: string) {
    ...
    this.setState((pre) => {
      ...
      data = data.updateIn(arr, (val) => {
        const active =
          val.get("active") === activeStatus.noSelect
            ? activeStatus.select
  ...
  checkPar (path: string) {
    ...
    const { selectArr, immutabData } = this.state;
    const indexArr = path.split(",").join(`,${childField!},`)
      .split(",");
    let _select = selectArr;
  ...
  mapDom (domArr: HTMLDivElement[]) {
    ...
    if (tabHArr[0] !== tabHArr[tabHArr.length - 1]) {
      // 高度不同
      ...
      [...viewTrArr[0]].map((val, index) => {
        ...
        if (trH[0] !== trHMax) {
          ...
          if (val.classList.contains("tree-td")) {
            const domArrs = trHdom.map(
              (vals) => vals.firstChild!.firstChild! as HTMLDivElement
            );
            this.mapDom(domArrs);
          } else {
            // 设置高度
    ...
    while (domArr.length) {
      ...
      domArr.forEach((arr) => {
        ...
        if (tabHArr[0] !== maxH) {
          ...
          const trArr_Arr = tabDomArr.map(
            (val) => val.lastElementChild!.children
          );

          [...trArr_Arr[0]].forEach((tr, index) => {
            const trDomArr = trArr_Arr.map((trArr) => trArr[index]);

            const trHArr = trDomArr
              .map((val) => val.clientHeight)
              .sort(compareFn);
  ...
  render () {
    ...
    return (
      <div className="treeTap-wrap" style={styleObj}>
        <div className="treeTab">{this.viewMap()}</div>
      </div>
    );
  }
}
