...
interface IBinTree<T> {
  createTreeNode(val:T):ITreeNode<T>;
}
class BinTree<T>  implements IBinTree<T> {
  root:ITreeNode<T> | undefined;
  constructor (data:T[]) {
  ...
  // 中序遍历 左 - 节点 - 右
  inOrderTraversal (node:ITreeNode<T>|undefined, arrContainer:T[]) {
  ...
  // 先序遍历 节点 - 左 - 右
  preOrderTraversal (node:ITreeNode<T>|undefined, arrContainer:T[]) {
